...
import type { Assign, Expand, IsAny } from './utils.cjs';
...
type RemoveRouteGroups<T extends string> = T extends `${infer Before}(${infer RouteGroup})${infer After}` ? RemoveRouteGroups<`${Before}${After}`> : T;
type NormalizeSlashes<T extends string> = T extends `${infer Before}//${infer After}` ? NormalizeSlashes<`${Before}/${After}`> : T;
type ReplaceFirstOccurrence<TValue extends string, TSearch extends string, TReplacement extends string> = TValue extends `${infer Prefix}${TSearch}${infer Suffix}` ? `${Prefix}${TReplacement}${Suffix}` : TValue;
