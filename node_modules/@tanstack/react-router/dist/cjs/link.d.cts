...
import type { Expand, IsUnion, MakeDifferenceOptional, NoInfer, NonNullableUpdater, PickRequired, Updater, WithoutEmpty } from './utils.cjs';
...
export type SearchPaths<TPaths, TSearchPath extends string> = TPaths extends `${RemoveTrailingSlashes<TSearchPath>}/${infer TRest}` ? TRest : never;
export type SearchRelativePathAutoComplete<TTo extends string, TSearchPath extends string, TPaths, TSearchedPaths = SearchPaths<TPaths, TSearchPath>> = TSearchedPaths extends string ? `${TTo}/${TSearchedPaths}` : never;
...
export type AbsolutePathAutoComplete<TFrom extends string, TPaths> = (string extends TFrom ? './' : TFrom extends `/` ? never : SearchPaths<TPaths, TFrom> extends '' ? never : './') | (string extends TFrom ? '../' : TFrom extends `/` ? never : '../') | TPaths | (TFrom extends '/' ? never : SearchPaths<TPaths, TFrom>);
